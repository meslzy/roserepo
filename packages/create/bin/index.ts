...
import templates, { useTemplate, Template } from "./templates";
...
const create = async () => {
	const {projectName} = await prompts({
		...
		initial: "my-project",
		format: ( value ) => value.toLowerCase().replace(/[^a-z0-9-]/g, "-"),
	});
	...
	await useTemplate(template, projectName, projectLocation);
};
