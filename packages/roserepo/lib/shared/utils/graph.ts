const findLastIndex = <T>( array: T[], predicate: ( item: T, index: number ) => boolean ): number => {
	...
	return founds.length === 0 ? -1 : founds[ founds.length - 1 ];
};
...
class Graph<T> {
	...
	private * getAdjacencyVertices(): Generator<[ Vertex<T>, Vertex<T> ]> {
		for ( const vertex of this.vertices ) {
			for ( const edgesId of vertex.edges ) {
				const adjacentVertex = this.vertices.find(( vertex ) => vertex.id === edgesId);
	...
	private * getDeepVertices( deep: "parents" | "children", vertex: Vertex<T>, deepVertex: Vertex<T>, alreadyVisited: VertexId[] = [] ): Generator<VertexId> {
		...
		if ( deep === "children" ) {
			...
			nextDependencies = this.getParents(deepVertex.id).map(( parent ) => parent.id);
		}

		for ( const nextDependencyId of nextDependencies ) {
			const nextDependencyVertex = this.vertices.find(( vertex ) => vertex.id === nextDependencyId);
	...
	* getDeepParents( id: VertexId ): Generator<Vertex<T>> {
	...
	getChildren( id: VertexId ): Vertex<T>[] {
		...
		if ( ! parentVertex || ! parentVertex.edges.length ) {
			return [];
		}
	...
	* getDeepChildren( id: VertexId ): Generator<Vertex<T>> {
	...
	topological2dSort(): Vertex<T>[][] {
		...
		Array.from(this.vertices.values()).forEach(( vertex ) => {
			...
			const parentsIds = Array.from(this.getDeepParents(vertex.id)).map(( parent ) => parent.id);
			const childrenIds = Array.from(this.getDeepChildren(vertex.id)).map(( child ) => child.id);

			const parentIndex = findFirstIndex(sort, ( vertices ) => vertices.some(( vertex ) => parentsIds.includes(vertex.id)));
			const childIndex = findLastIndex(sort, ( vertices ) => vertices.some(( vertex ) => childrenIds.includes(vertex.id)));
	...
	topologicalSort(): Vertex<T>[] {
		...
		Array.from(this.vertices).forEach(( vertex ) => {
			...
			const parentIndex = findFirstIndex(sort, ( vertex ) => parentsIds.includes(vertex.id));
			const childIndex = findLastIndex(sort, ( vertex ) => childrenIds.includes(vertex.id));
